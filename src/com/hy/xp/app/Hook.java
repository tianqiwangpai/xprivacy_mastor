...
public class Hook implements Comparable<Hook>
{
	...
	{
		String name = String.format("%s.%s.%s", PrivacyManager.cSettingDangerous, this.getRestrictionName(), this.getName());
	...
	public static boolean isAOSP(int sdk)
	{
		...
		if (Build.VERSION.SDK_INT >= sdk) {
			...
			if (Build.DISPLAY == null || Build.HOST == null)
				return false;
			return (isAOSP() || isCyanogenMod() || isOmni() || isMIUI() || isSlim() || isParanoidAndroid() || isCarbon() || isDirtyUnicorns() || isLiquidSmooth() || isAndroidRevolutionHD() || isMahdi());
		} else
			return false;
	}
