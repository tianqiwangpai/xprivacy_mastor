...
public class Meta
{
	...
	{
		return (cTypeAddress.equals(type) || cTypeAction.equals(type) || cTypeCommand.equals(type) || cTypeFilename.equals(type) || cTypeIPAddress.equals(type) || cTypeLibrary.equals(type) || cTypeMethod.equals(type) || cTypePermission.equals(type) || cTypeProc.equals(type) || cTypeTransaction.equals(type) || cTypeUrl.equals(type));
	}
	...
	public static List<Hook> get()
	{
		...
		// um access_type:ACCESS_TYPE_2G_3G or access_type:ACCESS_TYPE_WIFI
		mListHook.add(new Hook("internet", "NetworkInfo.getState", null, 1, "2.2.2", "getState").unsafe());

		mListHook.add(new Hook("internet", "NetworkInfo.isConnected", null, 1, "2.2.2", "isConnected").unsafe().dangerous().whitelist(cTypeIPAddress));
		// mListHook.add(new Hook("internet", "NetworkInfo.isConnected", null,
		// 1, "2.2.2", "isConnected").unsafe());

		// 11-04 19:46:31.669: D/MobclickAgent(6459): network is unavailable
		mListHook.add(new Hook("internet", "NetworkInfo.isConnectedOrConnecting", null, 1, "2.2.2", "isConnectedOrConnecting").unsafe().dangerous().whitelist(cTypeIPAddress));
		// mListHook.add(new Hook("internet",
		// "NetworkInfo.isConnectedOrConnecting", null, 1, "2.2.2",
		// "isConnectedOrConnecting").unsafe());
		...
		// new add start
		mListHook.add(new Hook("identification", "SERIAL", "", 1, null, null).restart().noUsageData());
		mListHook.add(new Hook("identification", "MODEL", "", 1, null, null).restart().noUsageData());
		mListHook.add(new Hook("identification", "MANUFACTURER", "", 1, null, null).restart().noUsageData());
		mListHook.add(new Hook("identification", "PRODUCT", "", 1, null, null).restart().noUsageData());
		mListHook.add(new Hook("identification", "RELEASE", "", 1, null, null).restart().noUsageData());
		mListHook.add(new Hook("identification", "ID", "", 1, null, null).restart().noUsageData());
		// new add end
