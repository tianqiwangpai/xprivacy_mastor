...
{
	public static final String AUTHORITY = "com.hy.xp.app.provider";
	...
	private Cursor queryRestrictions(final int uid, final String restrictionName, final String methodName, boolean usage)
	{
		...
		if (uid == 0) {
			...
			for (ApplicationInfo appInfo : pm.getInstalledApplications(PackageManager.GET_META_DATA)) {
				SharedPreferences prefs = getContext().getSharedPreferences(PREF_RESTRICTION + "." + appInfo.uid, Context.MODE_WORLD_READABLE);
	...
	{
		SharedPreferences prefs = getContext().getSharedPreferences(PREF_USAGE + "." + restrictionName, Context.MODE_PRIVATE);
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs)
	{
		if (sUriMatcher.match(uri) == TYPE_RESTRICTION) {
			...
			Util.log(null, Log.INFO, String.format("Update usage data %d/%s/%s=%b", uid, restrictionName, methodName, restricted));
	...
	private static void updateRestriction(Context context, int uid, String restrictionName, String methodName, boolean allowed)
	{
		// Update restriction
		SharedPreferences prefs = context.getSharedPreferences(PREF_RESTRICTION + "." + uid, Context.MODE_WORLD_READABLE);
	...
	private void updateUsage(final int uid, final String restrictionName, final String methodName, final boolean restricted, long timeStamp)
	{
		SharedPreferences prefs = getContext().getSharedPreferences(PREF_USAGE + "." + restrictionName, Context.MODE_PRIVATE);
		SharedPreferences.Editor editor = prefs.edit();
		...
		try {
			...
			synchronized (mFallbackRestrictionLock) {
				if (mFallbackRestrictions == null || mFallbackRestrictionsUid != uid) {
					...
					Util.log(null, Log.INFO, "Load fallback restrictions uid=" + uid + "/" + mFallbackRestrictionsUid + " " + ms + " ms");
		...
		try {
			...
			synchronized (mFallbackSettingsLock) {
				...
				if (mFallbackSettingsTime + PrivacyManager.cSettingCacheTimeoutMs < now) {
					...
					if (log) {
						long ms = System.currentTimeMillis() - now;
						Util.log(null, Log.INFO, "Reload fallback settings uid=" + Binder.getCallingUid() + " " + ms + " ms");
					}
				}
			}
	...
	{
		return Util.getUserDataDirectory(Process.myUid()) + File.separator + "shared_prefs" + File.separator + preference + ".xml";
	}

	private static String getPrefFileName(String preference, int uid)
	{
		return Util.getUserDataDirectory(uid) + File.separator + "shared_prefs" + File.separator + preference + "." + uid + ".xml";
	}
	...
	public static void fixFilePermissions()
	{
		...
		if (files != null)
			for (File file : files)
				if (file.getName().startsWith("com.hy.xp.app.provider.") && file.getName().endsWith(".xml") && !file.getName().contains(".usage."))
	...
	{
		File source = new File(Util.getUserDataDirectory(Process.myUid()) + File.separator + "shared_prefs" + File.separator + "com.hy.xp.app.provider.xml");
		...
		if (prefFile.exists() && !migratedFile.exists()) {
			...
			for (String settingKey : prefs.getAll().keySet())
				try {
					...
					SharedPreferences aprefs = context.getSharedPreferences(PREF_SETTINGS + "." + uid, Context.MODE_WORLD_READABLE);
	...
	public static List<PSetting> migrateSettings(Context context, int uid)
	{
		...
		if (prefFile.exists() && !migratedFile.exists()) {
			...
			SharedPreferences prefs = context.getSharedPreferences(PREF_SETTINGS + "." + uid, Context.MODE_WORLD_READABLE);
