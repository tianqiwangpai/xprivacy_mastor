...
public class XBinder extends XHook
{
	...
	// @formatter:off
	public static List<String> cServiceOptional = Arrays.asList(new String[] { "<noname>", "iphonesubinfo", "iphonesubinfo_msim", "sip", "isms", "nfc", "bluetooth", "bluetooth_manager" });
	...
	// private boolean execTransact(int code, int dataObj, int replyObj, int
	// flags)
	...
	private void markIPC(XParam param) throws Throwable
	{
		...
		if (ok) {
			...
			if ((flags & ~FLAG_ALL) != 0)
				Util.log(this, Log.ERROR, "Unknown flags=" + Integer.toHexString(flags) + " descriptor=" + descriptor + " code=" + code + " uid=" + Binder.getCallingUid());
			flags |= (mToken << BITS_TOKEN);
			param.args[3] = flags;
		} else {
			int level = (found ? Log.WARN : Log.ERROR);
			Util.log(this, level, "Unmarked descriptor=" + descriptor + " found=" + found + " code=" + code + " uid=" + Binder.getCallingUid());
	...
	private void checkIPC(XParam param) throws Throwable
	{
		...
		if (token != mToken) {
						...
						for (Field field : clazz.getDeclaredFields())
							try {
								if (field.getName().startsWith("TRANSACTION_") || field.getName().endsWith("_TRANSACTION")) {
									...
									String txName = field.getName().replace("TRANSACTION_", "").replace("_TRANSACTION", "");
			...
			if (codeName == null) {
				...
				Util.log(this, Log.WARN, "Unknown transaction=" + descriptor + ":" + code + " class=" + param.thisObject.getClass() + " uid=" + Binder.getCallingUid());
				Util.logStack(this, Log.INFO);
			}

			Util.log(this, Log.INFO, "can restrict transaction=" + interfaceName + ":" + codeName + " flags=" + flags + " uid=" + uid + " my=" + Process.myUid());
	...
	{
		return (code == PING_TRANSACTION || code == DUMP_TRANSACTION || code == INTERFACE_TRANSACTION || code == TWEET_TRANSACTION || code == LIKE_TRANSACTION || code == SYSPROPS_TRANSACTION);
