...
public class XIntentFirewall extends XHook
{
	...
	// resolvedComponent, int intentType, Intent intent, int callerUid, int
	// callerPid, String resolvedType, int receivingUid)
		...
		if (mapIntentRestriction.containsKey(action)) {
			...
			if (Intent.ACTION_NEW_OUTGOING_CALL.equals(action)) {
				...
				if (intent.hasExtra(TelephonyManager.EXTRA_INCOMING_NUMBER)) {
					...
					if (phoneNumber != null) {
						if (isRestrictedExtraValue(uid, restrictionName, action, phoneNumber, phoneNumber))
							intent.putExtra(TelephonyManager.EXTRA_INCOMING_NUMBER, (String) PrivacyManager.getDefacedProp(Binder.getCallingUid(), "PhoneNumber"));
					}
				}
