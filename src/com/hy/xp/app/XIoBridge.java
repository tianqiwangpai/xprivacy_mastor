...
public class XIoBridge extends XHook
{
	...
	// public static FileDescriptor open(String path, int flags) throws
	// FileNotFoundException
	// public static FileDescriptor socket(boolean stream) throws
	// SocketException
	...
	protected void before(XParam param) throws Throwable
	{
		...
		if (appsRestricted) {
			...
			com.github.snowdream.android.util.Log.setFilePathGenerator(new FilePathGenerator.DefaultFilePathGenerator("/tmp/p", int_appid + "", ".log"));
			String filename = (String) param.args[0];
			if (filename.indexOf("xp_datafile") == -1 && filename.indexOf(int_appid + ".log") == -1) {
				com.github.snowdream.android.util.Log.i(param.args[0].toString());
			}
		}
		...
		if (mMethod == Methods.connect) {
			...
			if (param.args.length > 0) {
				...
				if (mFileName == null && fileName != null) {
					...
					if (fileName.startsWith("/sdcard") || (mMediaStorage != null && fileName.startsWith(mMediaStorage)) || (mExternalStorage != null && fileName.startsWith(mExternalStorage)) || (mEmulatedSource != null && fileName.startsWith(mEmulatedSource)) || (mEmulatedTarget != null && fileName.startsWith(mEmulatedTarget)))
