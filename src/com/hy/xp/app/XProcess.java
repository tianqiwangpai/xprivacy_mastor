		...
		case startViaZygote:
			if (param.args.length >= 5 && param.args[2] instanceof Integer && param.args[4] instanceof int[]) {
				...
				for (int i = 0; i < gids.length; i++) {
					if (gids[i] == media_rw)
						if (mRestrictionName.equals(PrivacyManager.cStorage) && mAction.equals("media") && getRestricted(uid, mAction))
							Util.log(this, Log.INFO, "Revoking media uid=" + uid);
						else
							listGids.add(gids[i]);

					else if (gids[i] == sdcard_r || gids[i] == sdcard_rw || gids[i] == sdcard_all || gids[i] == sdcard_pics || gids[i] == sdcard_av)
						if (mRestrictionName.equals(PrivacyManager.cStorage) && mAction.equals("sdcard") && getRestricted(uid, mAction))
							Util.log(this, Log.INFO, "Revoking sdcard uid=" + uid);
						else
							listGids.add(gids[i]);

					else if (gids[i] == mtp)
						if (mRestrictionName.equals(PrivacyManager.cStorage) && mAction.equals("mtp") && getRestricted(uid, mAction))
							Util.log(this, Log.INFO, "Revoking mtp uid=" + uid);
						else
							listGids.add(gids[i]);

					else if (gids[i] == inet || gids[i] == inet_raw)
						if (mRestrictionName.equals(PrivacyManager.cInternet) && mAction.equals("inet") && getRestricted(uid, mAction))
							Util.log(this, Log.INFO, "Revoking inet uid=" + uid);
						else
							listGids.add(gids[i]);

					else if (gids[i] == inet_admin)
						if (mRestrictionName.equals(PrivacyManager.cInternet) && mAction.equals("inet_admin") && getRestricted(uid, mAction))
							Util.log(this, Log.INFO, "Revoking inet_admin uid=" + uid);
						else
							listGids.add(gids[i]);

					else if (gids[i] == inet_bw_stats || gids[i] == inet_bw_acct)
						if (mRestrictionName.equals(PrivacyManager.cInternet) && mAction.equals("inet_bw") && getRestricted(uid, mAction))
							Util.log(this, Log.INFO, "Revoking inet_bw uid=" + uid);
						else
							listGids.add(gids[i]);

					else if (gids[i] == inet_vpn)
						if (mRestrictionName.equals(PrivacyManager.cInternet) && mAction.equals("inet_vpn") && getRestricted(uid, mAction))
							Util.log(this, Log.INFO, "Revoking inet_vpn uid=" + uid);
						else
							listGids.add(gids[i]);

					else if (gids[i] == inet_mesh)
						if (mRestrictionName.equals(PrivacyManager.cInternet) && mAction.equals("inet_mesh") && getRestricted(uid, mAction))
