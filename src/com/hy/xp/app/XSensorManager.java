...
public class XSensorManager extends XHook
{
	...
	// rateUs, int maxBatchReportLatencyUs)
	// boolean registerListener(SensorEventListener listener, Sensor sensor, int
	// rateUs, Handler handler)
	// boolean registerListener(SensorEventListener listener, Sensor sensor, int
	// rateUs, int maxBatchReportLatencyUs, Handler handler)
	// boolean registerListener(SensorEventListener listener, Sensor sensor, int
	// rateUs)
	...
	private boolean isRestricted(XParam param, int type) throws Throwable
	{
		if (type == Sensor.TYPE_ALL)
			...
			if (isRestricted(param, "motion"))
				return true;
		} else if (type == Sensor.TYPE_ORIENTATION || type == Sensor.TYPE_GYROSCOPE || type == Sensor.TYPE_GYROSCOPE_UNCALIBRATED) {
			...
			if (isRestricted(param, "proximity"))
				return true;
		} else if (type == Sensor.TYPE_GAME_ROTATION_VECTOR || type == Sensor.TYPE_GEOMAGNETIC_ROTATION_VECTOR || type == Sensor.TYPE_ROTATION_VECTOR) {
